...
class IConnectTemplateRepository(Protocol):
    ...
    def list_all(self, limit: int = 100, published_only: bool = True) -> List[ConnectTemplate]: ...
...
class IConnectMessageRepository(Protocol):
    ...
    def list_all(self, limit: int = 100, skip: int = 0) -> List[ConnectMessage]: ...
...
class ConnectTemplateRepository(IConnectTemplateRepository):
    ...
    def list_all(self, limit: int = 100, published_only: bool = True) -> List[ConnectTemplate]:
        query = self.db.query(ConnectTemplate)
        if published_only:
            query = query.filter(ConnectTemplate.published == True)
        return query.order_by(ConnectTemplate.created_at.desc()).limit(limit).all()
...
class ConnectMessageRepository(IConnectMessageRepository):
    ...
    def list_all(self, limit: int = 100, skip: int = 0) -> List[ConnectMessage]:
        return self.db.query(ConnectMessage).order_by(
            ConnectMessage.created_at.desc()
